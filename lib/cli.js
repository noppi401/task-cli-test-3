...
function parseListFilter(args) {
  ...
  const index = args.findIndex((option) => option === '--filter' || option === '-f');
...
function formatTasks(tasks) {
  ...
  const widths = ['ID', 'Title', 'Status', 'CreatedAt'].map((label, index) => Math.max(label.length, ...rows.map((row) => row[index].length))));
